...
class StepEight_OC_P2 : AppCompatActivity(), Detector.DetectorListener {
    ...
    private fun setupButtonListeners() {
        bckButton.setOnClickListener {
            startActivity(Intent(this, StepEight_OC_P1::class.java))
    ...
    override fun onDetect(boundingBoxes: List<BoundingBox>, inferenceTime: Long) {
        runOnUiThread {
            ...
            val usb20Boxes = boundingBoxes.filter { it.clsName == " USB2.0 interface" }
            val usb30Boxes = boundingBoxes.filter { it.clsName == " USB3.0 interface" }
            val usb40Boxes = boundingBoxes.filter { it.clsName == " Jumper interface" }
            val usb50Boxes = boundingBoxes.filter { it.clsName == " Motherboard baffle" }

            // Combine all bounding boxes into one list to display
            val boxesToDisplay = usb20Boxes + usb30Boxes + usb40Boxes + usb50Boxes

            // Update UI based on detection
            binding.boundingBoxOverlay.setBoundingBoxes(boxesToDisplay)
            if (boxesToDisplay.isNotEmpty()) {
                statusTextView.text = "PLUG CONNECTIONS HERE"
                nextButton.visibility = View.VISIBLE
    ...
    private fun goToNextInterface() {
        val intent = Intent(this, FinalChecking_P1::class.java)
